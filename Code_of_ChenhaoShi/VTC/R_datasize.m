...
for idx = 1:size(combinations, 1)
    ...
    for d_idx = 1:num_d_values
        ...
        mask = (err_1_matrix <= 1e-1) & (M < 1000) & (aoi_1 <= aoi_th);
    ...
    % 绘制曲线
    plot(d_range, max_sum_tau_values*1e4);
    
    % 添加图例
    legends{idx} = sprintf('AoI_th=%d, SNR_1=%d, SNR_2=%d', aoi_th, SNR_1, SNR_2);
end
